...
{
    class BackupStream : IDisposable
    {
        ...
        public IEnumerable<Win32FileStreamHeader> Streams
        {
            get
            {
                var header = default(Win32FileStreamHeader);
                while (Read(ref header))
                {
                    yield return header;
                    Seek(in header);
                }
        ...
        public BackupStream(string filePath) => this.fileHandle = CreateFile(new UNCPath(filePath), NativeFileAccess.GenericRead, FileShare.Read, IntPtr.Zero, FileMode.Open, NativeFileFlags.BackupSemantics, IntPtr.Zero);
